...
- (void)viewDidLoad {
    ...
    [self initView];
    //设置下拉刷新
    __block WifiClockViewController *tmp = self;
...
-(void)initView
{
    ...
    [container addSubview:line6];
    [line6 setTimes:times];
    
    line7 = [[TimeLineView alloc]init];
    [container addSubview:line7];
    ...
    [container addSubview:lable6];
    lable6.text = @"星期六";
    
    lable7 = [[UILabel alloc]init];
    [container addSubview:lable7];
    lable7.text = @"星期天";
    
    //设置frame
    //wifi状态提示文本
    wifiStateText.frame = CGRectMake(0,  verticalSpace, self.view.frame.size.width, 20);
    //星期一
    line1.frame = CGRectMake(textSize.width + left_right_margin, CGRectGetMaxY(wifiStateText.frame)  , self.view.frame.size.width - 2*left_right_margin - textSize.width, line1.getHeight);
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        if(!error)
        {
            dispatch_sync(dispatch_get_main_queue(), ^{
                ...
                {
                    toggle.on = NO;
                    wifiStateText.text = @"wifi定时功能已经关闭";
                    [self.view addSubview:over];
                }
                else
                {
                    toggle.on = YES;
                    wifiStateText.text = @"wifi定时功能已经开启";
        ...
        {
            [self performSelectorOnMainThread:@selector(showFailedView:) withObject:@"网络故障，获取wifi定时信息失败" waitUntilDone:YES];
        }
...
-(void)viewWillAppear:(BOOL)animated
{
    if(_isNeedRefresh)
    {
        [MBProgressHUD showMessage:@"正在获取最新wifi定时信息"];
        [self refresh];
    }
    else
    {
...
-(void)wifi_toggle:(UISwitch*)paramSender
{
    ...
    //发送设置命令
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        BOOL ret = [[SHRouter currentRouter] setUpWifiClock:[SHRouter currentRouter].wlanSchedState ?@1 : @0 wlanSched:wifiClockJson error:nil];
        if(!ret)
        {
            ...
            [self performSelectorOnMainThread:@selector(showFailedView:) withObject:@"网络故障，设置失败" waitUntilDone:YES];
        }
        else
        {
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                {
                    wifiStateText.text = @"wifi定时功能已经关闭";
                    [MessageUtil showShortToast:@"已关闭wifi定时功能"];
                    [self.view addSubview:over];
                }
                else
                {
                    wifiStateText.text = @"wifi定时功能已经开启";
                    [MessageUtil showShortToast:@"已开启wifi定时功能"];
                    [over removeFromSuperview];
                }
            });
